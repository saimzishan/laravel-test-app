...
class FirmSync extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'firm:sync {firm_id} {--type=manual} {--debuging=false} {--only=all}';
    ...
    private function syncPP () {
        FirmIntegration::where("firm_id",$this->firm->id)->update(["status_message"=>"Sync is in progress. Depending on the size of your law firm, the timing could vary."]);
    ...
    private function generateOutput () {
        ...
        if (!$this->error) {
            ...
            if ($this->summaryGenerate){
                $this->firm->integrationRelation->percentage = 90;
                FirmIntegration::where("firm_id",$this->firm->id)->update(["status_message"=>"Your dashboard reports are now being created. Depending on the amount of data, generating time will vary."]);
                $this->firm->push();
                FirmSummaryGenerateJob::dispatch($this->firm->id, "", $this->firstTime)->onQueue("summary1");
                FirmIntegration::where("firm_id",$this->firm->id)->update(["status_message"=>"Sync Completed Successfully.","percentage"=>100]);
